...
function finishOrder(event)
{
	...
	var confirmation = confirm("Send Order to Kitchen?");
...
function populateMenu() 
{
    ...
    $.getJSON( '/tender/menu', function( data ) {
        ...
        $.each(menuData, function(){
            ...
            tableContent += '<td><a href="#" class="linkadditem" rel="' + this.id + '">' + this.name + '</a></td>';
...
function populateOrder()
{
	...
	$.each(order[2], function()
	{
		console.log(this);
		$.each(this, function()
		{
			orderString += this.name + "<br>";
		});
		orderString += "<br>";
	});
	if (order[3] != null)
...
function selectTable()
{
	...
	var tablenum = parseInt($(this).attr('rel'));
...
function addNotes(event)
{
	...
	if (confirmation === true)
	{
		...
		populateOrder();
		$('#buttons').html("<form method='post' action='/addcourse'><input id='orderout'></input><button type='submit' class='finishOrder'>Finish Order</button></form>");
	}
}
